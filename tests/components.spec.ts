...
test.describe('Component Tests', () => {
  test('TaskCard should render correctly', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="task-title"]')).toHaveText('Test Task');
  ...
  test('TaskForm should handle input correctly', async ({ page }) => {
    ...
    await page.fill('[data-testid="task-description-input"]', 'New Description');
    await page.fill('[data-testid="task-due-date-input"]', '2024-12-31');
    
    // Verify input values
    await expect(page.locator('[data-testid="task-title-input"]')).toHaveValue('New Task');
    await expect(page.locator('[data-testid="task-description-input"]')).toHaveValue('New Description');
    await expect(page.locator('[data-testid="task-due-date-input"]')).toHaveValue('2024-12-31');
  });
